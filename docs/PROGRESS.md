...
- **🧠 SIRADAKİ:** **W2 (deep-nesting → otonom flatten, UÇUŞTA)** aynı pattern (extract-guard-clause lander ekle + flip; `feat(bridge)` scope'la commit → concrete sayılır). Sonra W3/W4 (god-class/coordinator — provable-seam gate), W5/W6 (alt-hamle + dürüst scaffold-first). Küçük 2. dalga: assist coupling→deps rotası + extract-constant fitness reporting (F2). Görev #94: bridge impact-scope `verified` vs `coverage_verifies` reconciliation (denetçi-flag, pre-existing, gelecek hardening).
