...
| 2026-07-05 | **Make visible — the single pane of glass.** `apex dashboard` gains two "living-cell" cards so all 7 layers appear on one deterministic HTML page: **📓 Memory** (the full N/7 vault roll-up, L5) and **🟨 JS/TS scope** (module graph / hubs / untested, L1) — the two genuine gaps (L4 Dream is already the "💤 Last dream" card). Both self-gate to `""` on a fresh / Python-only project (byte-identical to before) and render deterministically. A live *dream-preview* card was rejected on evidence: `dream_confluence_modules` measured **184 s**, which would break the dashboard's speed and its deterministic-snapshot value. Format: rich HTML cards (not CLI text, not live-updating JS) — a byte-reproducible, shareable, proof-carrying photograph of the cell. | `app/reporting/dashboard.py`, `app/reporting/dashboard_sections.py`, `tests/test_dashboard_cards_eyml.py` |
| 2026-07-05 | **L6 grows a NATIVE intelligence (v0) — no external LLM, zero tokens.** Founder direction: don't rent a brain (external LLM proposal lane), *grow Apex's own*. `app/engine/native_synth.py` mines the project's OWN proven functions into a deterministic library of candidate bodies, adapts them (positional param remap) to an unfinished stub, and PROPOSES them — each still gated by `stub_synthesis`'s never-fake-green verifier, so nothing lands unverified. Proven end-to-end: learns `add(a,b)→a+b` from the corpus, proposes `x+y` for a stub `total(x,y)` with a `>>>` contract, the gate accepts it (and rejects `x*y`). Deterministic, offline, honest attribution `native-mind:<source>`. This is the founder's "small internal intelligence + core" made real. **Next slice:** wire as an opt-in extra candidate source into the `apex develop` implement-stub path (default off → byte-identical). | `app/engine/native_synth.py`, `tests/test_native_synth_eyml.py` |
