...
WAVE-1 SCOPE CUT (stated here by design): the (now possibly-unused)
``from typing import ...`` import line is left ALONE. Removing it would expand the
blast radius; the existing ``remove-unused-imports`` objective cleans it on a later
pass, and an unused import is behaviour-identical. Likewise an annotation with no
rewritable node is left byte-for-byte unchanged.
...
def _collect_edits(
    tree: ast.Module, rewriter: _Rewriter
) -> list[tuple[tuple[int, int, int, int], str]] | object:
    """Every ``(span, new_text)`` annotation-slot edit, in source order, or
    :data:`_REFUSE` when any slot hits a sharp edge.
    ...
    spelling is left byte-for-byte unchanged). A slot with no position info is
    skipped (it cannot be spliced safely)."""
...
def modernize_typing(source: str) -> str | None:
    ...
    result is re-``ast.parse``d (a malformed rewrite is dropped). Deterministic,
    stdlib-only, idempotent — a second run finds no legacy spelling and is a
    ...
    edits = _collect_edits(tree, rewriter)
    if edits is _REFUSE:
    ...
    for span, text in sorted(edits, key=lambda e: (e[0][0], e[0][1]), reverse=True):
        ...
        lines = spliced
    return rejoin_guarded(source, lines)
