...
def _dedent_else(lines: list[str], if_node: ast.If) -> list[str] | None:
    ...
    # Indentation step between the ``if`` head and its body, e.g. 4 spaces.
    ...
    # The ``else:`` keyword line is the last non-blank line strictly before the
    # body. It must carry exactly ``else:`` at the ``if`` indent.
    else_kw_idx = None
    ...
    # Anything other than blank lines between ``else:`` and its first statement
    # is a comment whose correct destination is ambiguous after a dedent. Refuse
    # rather than relocate it to a misleading indent.
    for i in range(else_kw_idx + 1, else_body_start):
        if lines[i].strip() != "":
            return None

    expected_body_indent = if_indent + step
    out: list[str] = []
