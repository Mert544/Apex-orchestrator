"""Self-registering objective: java-document-returns — a Java doc-surface landing that
mirrors the Python :mod:`document_returns` (a ``:returns:``/``Returns:`` section rendered
from a function's DECLARED ``-> T`` annotation) and the JS ``js-document-returns-inferred``
(a JSDoc ``@returns {T}``). It is the return-contract sibling of
:mod:`app.execution.objectives.java_document_param` (``@param``) and
:mod:`app.execution.objectives.java_document_throws` (``@throws``), completing the Java
Javadoc contract triad.

For an own non-test ``.java`` method that DECLARES a NON-``void``, NON-constructor return
type but carries NO Javadoc, java-document-returns lands a FRESH Javadoc block with one
``@return <type>`` line carrying the DECLARED return type (a byte-splice of a leading
``/** ... */`` comment just before the method's modifiers). The documented type is the
method's DECLARED return type (``MethodTree.getReturnType()``), read VERBATIM off the
type tree's SOURCE SPAN (the byte slice of the source between the tree's start/end offsets
— NOT ``Tree.toString()``, which can normalize generics/annotations/whitespace), so it is
byte-faithful to the declaration, NEVER inferred. Unlike the JS lane (where
``js-document-returns-inferred`` must INFER the type from literal ``return`` nodes), a Java
return type is DECLARED in the signature, so ``@return <type>`` is mandatory, meaningful,
non-empty content — exactly the way ``@param <name>`` carries the declared name and the way
the Python ``document-returns`` renders ``ast.unparse(fn.returns)`` verbatim. A Javadoc is
a COMMENT, so the edit changes ZERO declared structure (types/fields/method-signatures): it
is the soundest, smallest-blast-radius Java edit, BEHAVIOUR-IDENTICAL by construction.
...
MUST re-parse with the IDENTICAL structural fact-set (the same oracle java-document-param
/ java-document-throws / java-finalize-field rely on, where a ``final`` modifier changes
zero facts). The scope is deliberately narrow for soundness:

* Target ONLY a method with a NON-``void``, NON-constructor return type AND **no existing
  Javadoc** (insert a FRESH block). A method that ALREADY carries a Javadoc is REFUSED —
  editing / merging an existing Javadoc is out of scope (it risks dropping author prose
  and is not behaviour-trivial to keep identical), exactly as the Python ``document-returns``
  only documents the UNDOCUMENTED. This refusal is ALSO what keeps java-document-returns
  disjoint from java-document-param AND java-document-throws: all three target a no-Javadoc
  method and all three insert a leading Javadoc, so whichever LANDS first makes the method
  documented and the others then refuse it — no double block. (java-document-returns emits
  ``@return``-ONLY blocks, no ``@param``/``@throws``, so the three contracts stay
  orthogonal, exactly as the Python document-returns vs document-param vs document-raises
  are separate.)
* REFUSE a ``void``-returning method — a ``@return`` on a void method is content-free AND
  wrong, exactly as the Python ``document-returns`` refuses a ``None``/``NoReturn`` head and
  java-document-param refuses a zero-parameter method.
* REFUSE a constructor — a constructor has no return type
  (``MethodTree.getReturnType()`` is null), so there is nothing to document.
...
* The single-Java-project gate (a ``pom.xml`` / ``build.gradle`` marker at the root) —
  reused verbatim from java-document-param, so the objective is a clean NO-OP on a
  Python / JS tree.

The pipeline mirrors :mod:`app.execution.objectives.java_document_param` exactly — same
...
1. **DETECT** — only when a single ``pom.xml`` / ``build.gradle`` is at the root. Each own
   non-test ``.java`` source with at least one undocumented method that declares a
   non-``void`` return type is a candidate (the walk / source-suffix gate / test-file
   refusal are reused from :mod:`app.execution.lang.java_adapter`).
2. **PLAN** — :func:`plan_java_document_returns` reads the file, asks the driver
   (``ApexJavaDriver.java return-targets``) for each undocumented method's name + declared
   return-type text + the method-start offset, splices the indented Javadoc block
...
def documentable_return_targets(root: Path, rel: str) -> list[JavaReturnTarget]:
    """The undocumented non-``void`` return-type method targets in ``root/rel`` the
    objective would actually document (empty on a non-source/test target or on refuse).
    ...
    non-void / non-constructor / no-existing-Javadoc gates), mirroring
    :func:`app.execution.objectives.java_document_param.documentable_param_targets`."""
...
def render_return_javadoc(name: str, return_type: str, indent: str,
    ...
    at column ``indent``, joined with ``eol`` and ending with ``eol + indent`` so the
    method keeps its column AND the file's line ending.

    The block is ``/**`` / a ``<name>.`` summary line / a blank ``*`` separator / one
    ``* @return <type>`` line carrying the DECLARED return type (verbatim) / ``*/`` / the
    method's indentation. It splices at the method-START offset, where the method's own
    leading indentation is ALREADY in the source just before the offset — so the OPENING
    ``/**`` line carries NO indent (the existing indent precedes it); only the continuation
...
def splice_return_javadoc(source: str, targets: list[JavaReturnTarget]) -> str | None:
    """``source`` with a fresh ``@return`` Javadoc block inserted at each target's
    ...
    indentation via :func:`render_return_javadoc`), which alters ZERO declared structure.
    A target with an empty return-type text would render a content-free contract, so such
    a (driver-impossible) target is skipped rather than emitting a bare block.
    ...
    for target in sorted(targets, key=lambda t: t.insert_offset, reverse=True):
        ...
        block = render_return_javadoc(target.name, target.return_type, indent, eol)
        new_source = new_source[:off] + block + new_source[off:]
    return new_source if new_source != source else None


def detect_return_targets(project_root: str | Path) -> list[str]:
    """The own non-test Java source files that have at least one undocumented method
    declaring a non-``void`` return type, each pinned by the single-Java-project root-gate.
    ...
    return [rel for rel in _walk_files(root, _JAVA_SUFFIXES)
            if documentable_return_targets(root, rel)]


def plan_java_document_returns(project_root: str | Path, rel: str) -> RenamePlan:
    """Build the document-returns plan for ONE source file, or an empty no-op plan
    (an honest refusal).

    Refuses a non-Java-source / test / fixture write target outright
    (:func:`is_java_source`, via :func:`documentable_return_targets`). For each
    undocumented non-``void`` method it splices a fresh ``@return`` Javadoc block
    ...
    plan = RenamePlan(old=rel, new="java-document-returns")
    root = Path(project_root)
    targets = documentable_return_targets(root, rel)
    if not targets:
    ...
    documented = splice_return_javadoc(original, targets)
...
def _documentable_files(project_root: str | Path) -> list[str]:
    """The own source files ``plan_java_document_returns`` would actually change — i.e.
    ...
    java-document-param's self-validating count)."""
    root = Path(project_root)
    return [rel for rel in detect_return_targets(root)
            if plan_java_document_returns(root, rel).new_contents]


def fitness(project_root: str | Path) -> float:
    """Fitness = how many own Java sources still declare a non-``void`` method with no
    Javadoc this objective can document with an ``@return`` line. 0 means none remain."""
    return float(len(_documentable_files(project_root)))


def moves(project_root: str | Path) -> list:
    """One ``java_document_returns`` move per documentable source file. The
    ``operator="java_document_returns"`` literal lives HERE (in the objectives package) so
    ...
    root = Path(project_root)
    return [Move(
        operator="java_document_returns",
        target=f"{rel}:java-document-returns",
        description=f"document the declared return type in {rel} with @return Javadoc",
        build_plan=lambda r=rel: plan_java_document_returns(root, r),
...
# `apex develop --objective java-document-returns`.
...
# entry. It registers DIRECTLY (a Java objective), exactly as java-document-param /
# java-document-throws / java-finalize-field / js-wire-exports do.
register(ObjectiveSpec(name="java-document-returns", fitness=fitness, moves=moves,
