...
#
# The two reviewable manifest constants (:data:`SOUNDNESS_STRATEGY`, the per-objective
# soundness argument, and :data:`SCOPE_VERIFY_ALLOWLIST`, the audited impact-scope
# exception list) now live in the dependency-free leaf ``app.engine.soundness_manifest``
# and are RE-EXPORTED here so this module's public API is unchanged. They were moved out
# of this module because ``develop_registry.register`` must read ``SOUNDNESS_STRATEGY``
# at registration time to refuse an undeclared objective; importing THIS module from
# ``develop_registry`` would close a static import cycle (``develop_registry`` ->
# ``soundness_audit`` -> ``objective_compiler`` -> ``develop_registry``) the architecture
# grade penalises, whereas the leaf imports nothing from the engine, so both readers
# depend only on it. The forward tripwire still makes it impossible to leave a
# registered objective undeclared (a new objective MUST state its soundness argument in
# the manifest before this check can pass AND before it can register).
from app.engine.soundness_manifest import SCOPE_VERIFY_ALLOWLIST, SOUNDNESS_STRATEGY
