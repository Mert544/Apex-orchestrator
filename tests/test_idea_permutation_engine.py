...
# --- the `verify` lens (new development operator) ---------------------------

def test_verify_operator_registered_exactly_once():
    # The new lens is present once, matches the Operator contract (a `{x}`
    # placeholder), and carries a calibrated weight between its siblings.
    from app.engine.idea_permutation import DEVELOPMENT_OPERATORS

    matches = [op for op in DEVELOPMENT_OPERATORS if op.name == "verify"]
    assert len(matches) == 1
    verify = matches[0]
    assert "{x}" in verify.template
    # Calibration: cheaper than extend/harden (0.6), dearer than the
    # generalize/decouple structural tier (0.45) — a focused, self-contained lens.
    assert 0.45 < verify.feasibility < 0.6
    # Still a unique name and a well-formed weight, like every sibling.
    names = [op.name for op in DEVELOPMENT_OPERATORS]
    assert len(names) == len(set(names))
    assert 0.0 <= verify.feasibility <= 1.0


def test_verify_operator_wired_into_hints_and_facets():
    ...
    assert "verify" in _OPERATOR_HINTS and _OPERATOR_HINTS["verify"]
    assert _FACETS.get("verify")            # has a level-1 facet vocabulary
    assert _LENS_SCENARIOS.get("verify")    # has its own counterfactual scenarios


def test_engine_emits_verify_lens_idea(tmp_path):
    # Running the engine on a small synthetic project yields at least one idea
    # generated by the `verify` lens, phrased from its template.
    _project(tmp_path)
    rep = IdeaPermutationEngine(
        {"max_total_ideas": 80, "max_idea_depth": 2, "breadth": 9}, tmp_path
    ).run()
    verify_ideas = [i for i in rep.ideas if i.operator == "verify"]
    assert verify_ideas, "expected at least one idea from the verify lens"
    # The template phrasing ("Strengthen the proof that {x} ...") reaches the
    # idea's rationale; the title is the composed lens-chain (".. → Verify: x").
    assert any("Strengthen the proof" in i.rationale for i in verify_ideas)
    assert all("Verify" in i.title for i in verify_ideas)
    # The new lens obeys every shared invariant.
    assert all(0.0 <= i.value <= 1.0 for i in verify_ideas)
    assert all(i.source_facts for i in verify_ideas)
    assert all("verify" in i.operator_chain for i in verify_ideas)


def test_engine_with_verify_lens_is_deterministic(tmp_path):
    # Adding the lens does not break determinism: two runs are identical and
    # every produced idea keeps value in [0, 1].
    _project(tmp_path)
    cfg = {"max_total_ideas": 80, "max_idea_depth": 2, "breadth": 9}
    a = IdeaPermutationEngine(cfg, tmp_path).run()
    b = IdeaPermutationEngine(cfg, tmp_path).run()
    assert [i.title for i in a.ideas] == [i.title for i in b.ideas]
...
def test_verify_caveat_stays_on_lens(tmp_path):
    # Its hint/scenario keep the verify caveat interrogating correctness/proof,
    # not the generic "malformed input" fallback.
    _project(tmp_path)
    rep = IdeaPermutationEngine(
        {"max_total_ideas": 80, "max_idea_depth": 2, "breadth": 9}, tmp_path
    ).run()
    verify = next(i for i in rep.ideas if i.operator == "verify")
    joined = " ".join(verify.caveats).lower()
