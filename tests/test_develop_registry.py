...
def test_soundness_lock_does_not_break_import_or_objective_count():
    # The lock runs at import for every current objective; all 93 are already in
    # SOUNDNESS_STRATEGY (91 + java-final-parameter + add-functools-wraps), so
    # import must stay clean and the count unchanged (no import breakage, no
    # count-pin drift).
    from app.engine.objective_compiler import available_objectives
    from app.engine.soundness_audit import SOUNDNESS_STRATEGY
