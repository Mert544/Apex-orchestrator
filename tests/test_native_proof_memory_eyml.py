...
def test_experience_reranks_candidates(tmp_path, monkeypatch):
    ...
    record_native_landing(tmp_path, ["p0 - p1"])             # subtraction proved
    _native_mind_sources.cache_clear()
...
def test_end_to_end_native_landing_is_remembered(tmp_path, monkeypatch):
    ...
    # ...and the experience memory remembered the idiom shape
    assert "max(p1, min(p0, p2))" in decayed_reliability(tmp_path)
