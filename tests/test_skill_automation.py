...
def _make_context(project_root: Path, focus_branch: str | None = None) -> AutomationContext:
...
def test_project_scan_plan_runs_all_registered_steps(tmp_path: Path):
    _build_demo_project(tmp_path)
    runner = SkillAutomationRunner(build_default_registry())

    result = runner.run_plan("project_scan", _make_context(tmp_path))

    assert [step.step_name for step in result.steps] == ["profile_project", "decompose_objective", "run_research"]
    assert all(step.status == "ok" for step in result.steps)
    assert result.final_output["branch_map"]
    assert result.final_output["recommended_actions"]


def test_focused_branch_plan_reuses_memory_and_selected_branch(tmp_path: Path):
    _build_demo_project(tmp_path)
    runner = SkillAutomationRunner(build_default_registry())

    full_result = runner.run_plan("project_scan", _make_context(tmp_path))
    focus_branch = next(branch for branch in full_result.final_output["branch_map"] if branch.count('.') >= 2)

    focused_result = runner.run_plan("focused_branch", _make_context(tmp_path, focus_branch=focus_branch))

    assert len(focused_result.steps) == 1
    assert focused_result.steps[0].status == "ok"
    assert focused_result.final_output["focus_branch"] == focus_branch
    assert focused_result.final_output["focus_claim"] == full_result.final_output["branch_map"][focus_branch]
    assert all(branch == focus_branch or branch.startswith(focus_branch + ".") for branch in focused_result.final_output["branch_map"])
