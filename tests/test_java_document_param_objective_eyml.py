...
def test_parity_concrete_count_is_forty_four():
    ...
    assert len(buckets["CONCRETE"]) == 47  # java-final-parameter landed on top (47th CONCRETE)
...
def test_behavior_identical_on_java_undocumented_params_corpus_shape():
    ...
    # is sound on the sweep. ``only`` slices the (process-memoized) sweep to THIS
    # objective's row — byte-identical to its row in the full sweep (per-objective
